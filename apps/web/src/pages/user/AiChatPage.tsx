...
import { Send, Bot, Sprout } from 'lucide-react'
import { useMutation } from '@tanstack/react-query'
import { apiClient } from '@lib/api-client'
import { cn } from '@lib/utils'
import type { AiMessage, AiChatResponse } from '@khetly/types'
import { formatDate } from '@khetly/utils'

const SUGGESTIONS = [
  'Which crop gives best returns near Gurugram in October?',
  'What are the risks of renting farmland?',
  'How does the rental payment work?',
  'What should I check before booking a listing?',
]
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-10rem)] lg:h-[calc(100vh-6rem)] max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-gray-100 flex-shrink-0">
        ...
        <div>
          <h1 className="text-base font-medium text-gray-900">Khetly AI Assistant</h1>
          <p className="text-xs text-gray-500">Ask anything about renting farmland</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500 mb-5">Start by asking a question</p>
        ...
        {messages.map((msg) => (
          ...
          >
            <div className={cn(
              ...
              msg.role === 'user' ? 'bg-brand-600 text-white' : 'bg-brand-50'
            )}>
              {msg.role === 'user' ? '👤' : '🌿'}
            </div>
            <div className={cn(
      ...
      <div className="flex gap-2 pt-3 border-t border-gray-100 flex-shrink-0">
        <input
          ...
          placeholder="Ask about crops, farming, risks…"
